...
model = nn.RNN(nn.LSTMCell(N_actions))
initial_params = model.init(jrng, jnp.ones((N_MC, n_time_steps, N_states)))
...
# save own code
with open('recurrent.py') as f, open(directory + '/self.py', 'w') as d:
    d.write(f.read())
...
while epoch < N_epochs:
    ...
    for timestep in range(n_time_steps):
        ...
        logp = model.apply(nnparams, statelist[:,:(timestep+1),:])[:,-1,:]
        p = np.exp(logp)
        #print(np.shape(p))
        #exit()
