...
def ModifiedFanOut():   #https://jax.readthedocs.io/en/latest/_modules/jax/example_libraries/stax.html#FanOut
    #https://jax.readthedocs.io/en/latest/jax.example_libraries.stax.html#jax.example_libraries.stax.FanOut
    
    """Layer construction function separating time (0-th parameter of the state) and the states."""
    
    def init_fun(rng, input_shape):
        time_shape = input_shape[:-1] + (1,)
        angle_shape = input_shape[:-1] + (1,)
        rest_shape = input_shape[:-1] + (input_shape[-1]-2,)
        return ([time_shape, angle_shape, rest_shape], ())
        # return ([input_shape] * num, ()) #in the original Fanout function

    def apply_fun(params, inputs, **kwargs):
        time_input = inputs.take(indices=jnp.arange(1), axis=-1)                            #time_input = inputs[:, :1]
        angle_input = inputs.take(indices=jnp.arange(1,2), axis=-1)                            #angle_input = inputs[:, 1:2]
        remaining_inputs = inputs.take(indices=jnp.arange(2,jnp.shape(inputs)[-1]), axis=-1)#remaining_inputs = inputs[:, 2:]
        return [time_input, angle_input, remaining_inputs]
        #return [inputs] * num        #in the original FanOut function

    return init_fun, apply_fun


initialize_params, predict = stax.serial(
        ModifiedFanOut(),                       #split time and (x,p)
        stax.parallel(
            stax.serial(
                    stax.Dense(2),              #For extraction of 2 possible time scales
                    stax.elementwise(jnp.sin),  #Pass the time through a periodic function with a phase and frequency determined by the neural network
                ),
            stax.serial(
                    stax.Dense(2),              #For extraction of 2 possible time scales
                    stax.elementwise(jnp.sin),  #Pass the angle through a periodic function with a phase and frequency determined by the neural network
                ),
            stax.Identity,  # Do not change the phase state (x,p) in this layer
            ),
        stax.FanInConcat(axis=-1),  # Combine (t1, t2, x, p)
...
while epoch < N_epochs:
    ...
    if epoch==N_epochs-1:
        ...
        env.Render(('greedy',returns), probs, trajectory_numbers_to_show=range(N_MC))    #Output all trajectories in the final chart
