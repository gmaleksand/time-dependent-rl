...
while epoch < N_epochs:
    ...
    if epoch==N_epochs-1 or epoch==2500: # render the phase portrait on epoch 2500 as well, when the best performance was observed 
        ...
        if epoch==N_epochs-1:
            env.Render(('greedy',returns), probs)
        if epoch==2500:
            env.Render_best((epoch,returns),probs)
