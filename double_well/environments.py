...
class flex_env():
    ...
    def __init__(self, 
        ...
        self.set_seed(seed)
        # Constants
        self.solver = ode(odes.double_well_var) # cosh_fixed has no time dependence
        self.solver.set_integrator('dop853', rtol=1e-3, atol=1e-6)            #Use the scipy ODE integrator (default:dop853)

        self.x0 = -np.ones(N_MC)    # Initial state for double-well
    ...
    def step(self, action, done_before): #Performs one step in the environemnt.
        ...
        done_now = np.where(4*(x-1)**2 + p**2 < 0.05 , True, False)     #(Mask) Condition for finishing the trajectory
        ...
        #print(x)
        #exit()
        reward = (-4*(x-1)**2 - p**2) - 10*(1-done_now)
        #print(reward)
        #exit()
    ...
    def Render(self, label, probs, critic_preds = None, use_insets = True, trajectory_numbers_to_show = [0], show_arrows = False):
...
class var_env():
    ...
    def step(self, action, done_before): #Performs one step in the environemnt.
        ...
        #self.state[:,2] = self.t[self.current_step-1]*2*np.pi/0.5  # the oscillations of the potential have angular frequency 0.5
        self.state[:,2] = np.sin(0.5*self.t[self.current_step-1])  # double-well / cosh height
        self.state[:,3] = np.cos(0.5*self.t[self.current_step-1])  # double-well / cosh height
        #self.state[:,3] = np.sin(np.sqrt(2)*self.t[self.current_step-1])  # cosh width
        """
        self.state[:,0] = self.t[self.current_step-1]   # Add the time to the state without explicit information about the time dependence of the environment
        self.state[:,1] = p
...
class fixed_env():
    ...
    def __init__(self, 
        ...
        self.set_seed(seed)
        # Constants
        self.solver = ode(odes.double_well_fixed) # cosh_fixed has no time dependence
        self.solver.set_integrator('dop853', rtol=1e-3, atol=1e-6)            #Use the scipy ODE integrator (default:dop853)
