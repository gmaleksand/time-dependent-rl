...
class var_env():
    ...
    def __init__(self, 
                ...
                seed=0, 
                Fpower=0.1,
                reward_equation="np.abs(x)",   #Default reward equation
                t=10.,
                deltat = 0.1
                ):    #Initialize the environment.

        # defining states and actions
        self.N_states = 3   # (x,p,t)
        self.N_actions = 2
        ...
        self.set_seed(seed)
        # Constants
        self.solver = ode(ODEs.cosh_var_height) # cosh_fixed has no time dependence
        self.solver.set_integrator('dop853', rtol=1e-3, atol=1e-6)            #Use the scipy ODE integrator (default:dop853)
        self.reward_equation = reward_equation
    ...
    def step(self, action, done_before): #Performs one step in the environemnt.
        ...
        #self.state[:,2] = np.sin(np.sqrt(2)*self.t[self.current_step-1])  # cosh width
        

        #print(x)
        #exit()
        reward = eval(self.reward_equation)         #np.abs(x)
        #print(reward)
        #exit()
    ...
    def save_phase_diagram(self, label, probs):
        directory, epoch, returns = label
        phasefile = open(directory+'/{}_phase_diagram.txt'.format(epoch), 'wb')
