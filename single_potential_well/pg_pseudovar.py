...
while epoch < N_epochs:
    ...
    if epoch==N_epochs-1 or epoch==3282: # render the phase portrait on epoch 3282 as well, when the best performance was observed 
        ...
        if epoch==N_epochs-1:
            env.Render(('greedy',returns), probs)#              #Phase diagram
        if epoch==3282:
            env.Render((epoch,returns), probs)
