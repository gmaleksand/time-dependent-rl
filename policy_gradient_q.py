...
directory =  f'data/q_basic&t={begin_time:d}'
os.mkdir(directory)
...
def details_to_table():# add all relevant parameters of the simulation in the table of experiments
    ...
    ws.cell(row=19,column=col,value='q_basic')
...
# save own code
with open('policy_gradient_q.py') as f, open(directory + '/self.py', 'w') as d:
    d.write(f.read())
with open('qubit_env.py') as f, open(directory + '/env.py', 'w') as d:
    d.write(f.read())
